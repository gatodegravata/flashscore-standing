# @title Função: busca jogos futuros
# ==============================================================================
# CÉLULA: Extrair Jogos Futuros com curl_cffi (Rodadas restantes + Escudos)
# ==============================================================================
# Instale se ainda não tiver: !pip install curl_cffi
...
def get_fixtures_dataframe(fixtures_url: str) -> pd.DataFrame:
    """
    Baixa os jogos futuros do Flashscore usando curl_cffi (impersonate='chrome120')
    e faz o parse de Rodada, Data/Hora, Mandante, Visitante e URLs dos Escudos.
    """
    ...
    html = r.text
    # Localiza os feeds de jogos embutidos no SSR da página
    feeds = re.findall(r'SA÷1¬~ZA÷.*', html)
    if not feeds:
        raise Exception("Nenhum feed de jogos encontrado no HTML retornado.")

    # O bloco com maior volume de dados contém toda a lista dos jogos futuros
    ...
    df = df.drop_duplicates(subset=["Match_ID"]).sort_values(by=["Round_Num", "Date"]).reset_index(drop=True)
    return df
